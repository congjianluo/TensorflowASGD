...
class Lenet:
  def __init__(self):
    ...
    self.sgd_train_op = tf.train.GradientDescentOptimizer(self.lr).minimize(self.loss)
    self.avsgd = AsynchronousStochasticGradientDescent(self.lr)
    self.avsgd_train_op = self.avsgd.minimize(self.loss)
